    ...
    ${
      encounter.patient
        ? `<table><tbody>
            ...
            <tr><th>insurer</th><td>${escape(encounter.cover?.insurerName || 'n/a')}</td></tr>
            <tr><th>cover</th><td>${escape((encounter.cover?.coverage || []).join(', '))} · ${escape(
              encounter.cover?.insuranceModel || '',
            )}</td></tr>
            <tr><th>card</th><td>${escape(encounter.cover?.cardNumber || 'n/a')}</td></tr>
            <tr><th>AHV</th><td>${escape(encounter.cover?.administrativeNumber || 'n/a')}</td></tr>
          </tbody></table>`
        : ''
    }
  </div>`;
    ...
    ${
      imm
        ? `<table><tbody>
            ...
            <tr><th>given</th><td>${escape(imm.occurrence_date || 'n/a')}</td></tr>
...
async function showProjections(containerId, vct, claims) {
  ...
  $(containerId).innerHTML += `<div class="card">
    ...
    <p class="why">
      Built locally from the disclosed claims. Derived and not authoritative: the signed credential
      is the evidence. It contains only what the holder actually released.
    </p>
