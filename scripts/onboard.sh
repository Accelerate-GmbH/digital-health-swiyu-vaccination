...
# The DID is `.state.id` of the first log entry.
#
# Do NOT grep the file for a did:webvh string: the `parameters.method` field
# holds the literal "did:webvh:1.0", it appears before the identifier and a
# textual match silently returns the method version as if it were the DID.
# Everything downstream then looks fine and is wrong.
did_from_log() {
  head -1 "$1" | jq -r '.state.id // empty'
}

# We generate the three P-256 keys ourselves so each has one job:
#
#   assert-key-01  signs issued credentials
#   assert-key-02  signs the status list, which is republished daily and public
#   auth-key-01    signs the verifier's JWT-Secured Authorization Requests
#
# Left to itself `didtoolbox create` generates exactly one assertion key, which
# would force the credential signing key to double as the status list key. The
# Ed25519 DID *update* key is still generated by the toolbox into .didtoolbox/.
readonly KEY_NAMES=(assert-key-01 assert-key-02 auth-key-01)

generate_keys() {
  local dir="$1"
  local key   # not `name`: the caller uses that and bash has no block scope
  mkdir -p "$dir/keys"
  for key in "${KEY_NAMES[@]}"; do
    openssl ecparam -name prime256v1 -genkey -noout -out "$dir/keys/$key" 2>/dev/null \
      || die "openssl could not generate $key"
    chmod 600 "$dir/keys/$key"
    openssl ec -in "$dir/keys/$key" -pubout -out "$dir/keys/$key.pub" 2>/dev/null \
      || die "openssl could not derive the public key for $key"
  done
}

assert_key_path() {
  local dir="$1"
  [[ -f "$dir/keys/assert-key-01" ]] && { printf '%s' "$dir/keys/assert-key-01"; return; }
  # DIDs created before this script generated its own keys.
  [[ -f "$dir/.didtoolbox/assert-key-01" ]] && { printf '%s' "$dir/.didtoolbox/assert-key-01"; return; }
  die "no assertion private key for this actor under $dir"
}
...
cmd_preflight() {
  step "Preflight"
  local missing=0
  for tool in curl jq java openssl; do
...
cmd_did() {
  ...
  generate_keys "$dir"
  ok "three P-256 key pairs: ${KEY_NAMES[*]}"
  # didtoolbox resolves key paths and writes .didtoolbox/ relative to the
  # working directory, so run it from the actor's own directory.
  ( cd "$dir" && didtoolbox create \
      --identifier-registry-url "$registry_url" \
  ...
  [[ -n "$did" ]] || die "could not read .state.id out of $dir/did.jsonl"
  ok "$did"
  dim "    private keys: $dir/keys/ and the DID update key in $dir/.didtoolbox/"
  dim "    Back both up. They are not recoverable and without the update key the"
  dim "    DID can never be changed again."
...
# Emit the DIDs, verification methods and, on request, the keys themselves.
cmd_env() {
  local with_keys=""
  [[ "${1:-}" == "--with-keys" ]] && with_keys=yes
  ...
  {
    say "# Generated by scripts/onboard.sh."
    if [[ -n "$with_keys" ]]; then
      say "# CONTAINS PRIVATE KEYS. Mode 600, git-ignored. Move them into a secret"
      say "# manager before this goes anywhere near a shared machine."
    else
      say "# Private keys are NOT printed. Re-run with --with-keys to write .env.generated."
    fi

    local dir name did prefix
    for dir in "$STATE_DIR"/*/; do
      ...
      if [[ -n "$with_keys" ]]; then
        emit_key "${prefix}_SDJWT_KEY"       "$dir/keys/assert-key-01"
        emit_key "${prefix}_STATUS_LIST_KEY" "$dir/keys/assert-key-02"
        emit_key "${prefix}_AUTH_KEY"        "$dir/keys/auth-key-01"
      else
        say "# keys: $dir""keys/{assert-key-01,assert-key-02,auth-key-01}"
      fi
    done
  } >> "$out"
  ...
  if [[ -n "$with_keys" ]]; then
    say
    ok "wrote .env.generated (mode 600)"
    dim "    Merge it into .env. It is git-ignored; keep it that way."
  fi
}

# The generic components read PEM keys from a single-line env var, so real
# newlines become the two characters \ and n.
emit_key() {
  local var="$1" file="$2"
  if [[ ! -f "$file" ]]; then
    say "# $var: key file missing ($file)"
    return
  fi
  printf '%s=%s\n' "$var" "$(awk '{printf "%s\\n", $0}' "$file")"
}
