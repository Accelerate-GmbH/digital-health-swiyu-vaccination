...
| --- | --- | --- |
| ~~The legal basis on which each issuer acts: KVG/LAMal Art. 42a, OR Art. 958f, MedBG/LPMéd, EpG/LEp~~ | `governance-framework.md`, credential definitions | **Largely closed 2026-09-13** by a reading against Fedlex done outside this environment and reported in. KVG Art. 42a and OR Art. 958f are confirmed, the latter with a scope qualification now carried in the text; the MedBG and EpG citations were incomplete and are corrected at source. Deployment-specific conclusions still need qualified counsel. See the **R** sections in [source verification](source-verification.md) |
| The KVG analysis list as a laboratory's authorisation | `governance-framework.md`, `lab-report.ts` | Review by qualified counsel. It was not part of the 2026-09-13 reading |
...
| Coverage-measurement and outbreak-response claims | `public-health.md`, `business-case.md` | **Largely closed 2026-09-13** against FOPH and UZH EBPI primary sources, read outside this environment: the coordinating institute, collection since 1999, the age groups 2, 8 and 16, register-based random sampling, invitation by letter and the vaccination-record copy are all confirmed. Three claims are not covered and are listed in [source verification](source-verification.md): the three contact attempts, per-canton workflow, and what the sampling frame already holds |
...
| FHIR element paths and openEHR archetype bindings | Credential definitions, `positioning.md` | Review by HL7 Switzerland and openEHR Switzerland. The bindings match the openEHR CKM mirror and the CH VACD FSH source. **Partially closed 2026-09-13**: `ch-profile-conformance.test.ts` now pins the constraints checkable without a validator, and doing so found two structural defects, since fixed. Full validation cannot be run in this environment, because the FHIR package registry is not reachable from it, and and one deliberate non-conformance is documented |
